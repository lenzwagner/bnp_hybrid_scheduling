...
class BranchAndPrice:
    ...
    def __init__(self, cg_solver, branching_strategy='mp', search_strategy='dfs', verbose=True,
        ...
        Args:
            ...
            search_strategy: 'dfs' for Depth-First-Search.
        ...
        self.node_counter = 0
        self.open_nodes = []  # For DFS: list of IDs.
        ...
        self.logger.info(f"Search strategy: {'Depth-First (DFS)'}")
    ...
    def create_root_node(self):
        ...
        # Store node
        self.nodes[0] = node
        if self.search_strategy == 'wfs':
            # wfs: Just add node ID (no bounds needed)
            self.open_nodes.append(0)
        else:  # DFS
            # DFS: Just add node ID
            self.open_nodes.append(0)
        ...
        try:
            ...
            if master.Model.status == gu.GRB.OPTIMAL:
                ...
                if ip_obj < self.incumbent:
                    ...
                    self.logger.info(f"   Previous incumbent: {old_incumbent:.6f}")
                    self.logger.info(f"   Gap: {self.gap:.4%}")

                    # NEW: Fathom open nodes that are now dominated
                    fathomed_count = self._fathom_by_bound()
                    if fathomed_count > 0:
                        self.logger.info(f"   🔪 Fathomed {fathomed_count} open nodes by new incumbent\n")
    ...
    def should_fathom(self, node, lambdas_dict):
        ...
        Args:
            ...
            lambdas_dict: Dictionary of lambda variable assignments
        ...
        if node.is_integral:
            ...
            if node.lp_bound < self.incumbent:
                ...
                # NEW: Fathom open nodes that are now dominated by the new incumbent
                fathomed_count = self._fathom_by_bound()
                if fathomed_count > 0:
                    self.logger.info(f"   🔪 Fathomed {fathomed_count} open nodes by new incumbent\n")
        ...
        # Check 4: Already marked as fathomed
    ...
    def solve(self, time_limit=3600, max_nodes=10000):
        ...
        if self.search_strategy in ['wfs', 'dfs']:
            self.open_nodes.pop()  # Remove root node (will be re-added if needed)

        lp_bound, is_integral, frac_info, root_lambdas = self.solve_root_node()
        ll = lp_bound
        self.stats['nodes_explored'] = 1

        if self.search_strategy == 'wfs' and not is_integral:
            self.open_nodes.append(0)
        elif self.search_strategy == 'dfs' and not is_integral:
            self.open_nodes.append(0)
        ...
        print('ll', ll, is_integral, self.incumbent)
        ...
        while self.open_nodes and iteration < max_nodes:
            ...
            # ========================================
            if self.search_strategy == 'wfs':
                # wfs (Breadth-First): Process nodes in the order they were added
                # Pop from front of list (oldest node first)
                current_node_id = self.open_nodes.pop(0)
                self.logger.info(f"   [wfs] Selected Node {current_node_id} (breadth-first order)")
            ...
            if lp_bound > 34 and current_node_id > 35:
                print('Ligger', lp_bound, self.best_lp_bound, current_node_id)
                #sys.exit()
            if current_node_id > 34:
                print('Ligger', lp_bound, self.best_lp_bound, current_node_id)
    ...
    def branch_on_mp_variable(self, parent_node, branching_info):
        ...
        # -------------------------
        if self.search_strategy == 'wfs':
            # wfs: Simply add nodes in order (no bounds needed)
            self.open_nodes.append(left_child.node_id)
            self.open_nodes.append(right_child.node_id)
            self.logger.info(f"    - [wfs] Added Nodes {left_child.node_id}, {right_child.node_id} to queue")

        else:  # DFS
            # DFS: Add in reverse order for left-first exploration
            self.open_nodes.append(right_child.node_id)  # Right first (bottom of stack)
            self.open_nodes.append(left_child.node_id)  # Left second (top of stack)
            self.logger.info(f"    - [DFS] Added Nodes to stack (next: {left_child.node_id})")
    ...
    def _fathom_by_bound(self):
        ...
        This method is called after the incumbent is improved to prune
        the search tree by eliminating nodes that cannot lead to better solutions.

        For DFS: Removes nodes from the stack

        Returns:
            int: Number of nodes fathomed
        """
        if self.incumbent == float('inf'):
            # No incumbent yet, cannot fathom by bound
            return 0

        fathomed_count = 0
        nodes_to_remove = []

        # Get list of node IDs to check based on search strategy
        ...
        for node_id in nodes_to_check:
            ...
            # Check if node's LP bound is worse than or equal to incumbent
            if node.lp_bound >= self.incumbent - 1e-5:
                node.status = 'fathomed'
                node.fathom_reason = 'bound_after_incumbent_update'
                self.stats['nodes_fathomed'] += 1
                nodes_to_remove.append(node_id)
                fathomed_count += 1

                self.logger.debug(f"     Fathomed node {node_id}: "
                                  f"LP={node.lp_bound:.6f} >= Inc={self.incumbent:.6f}")

        # Remove fathomed nodes from open list
        if nodes_to_remove:
            self.open_nodes = [i for i in self.open_nodes
