...
class BranchAndPrice:
    ...
    def solve(self, time_limit=3600, max_nodes=10000, solve_compact=None):
        """
        ...
        if not self.open_nodes:
            ...
            # ========================================
            # OPTIONAL: SOLVE COMPACT MODEL
            # ========================================
            compact_results = None
            if solve_compact:
                self._print("\n" + "=" * 100)
                self._print(" COMPACT MODEL COMPARISON ".center(100, "="))
                self._print("=" * 100 + "\n")

                compact_results = self.solve_compact_model_for_comparison()
                self.compare_with_compact_model(compact_results)

            # Create results dict
            results = self._get_results_dict()
            if compact_results:
                results['compact_results'] = compact_results
    ...
    def solve_compact_model_for_comparison(self):
        """
        Solve the compact model for comparison with B&P results.

        This provides a benchmark to verify that B&P found the optimal solution.

        Returns:
            dict: Results from compact model solve
        """
        self._print("\n" + "=" * 100)
        self._print(" SOLVING COMPACT MODEL FOR COMPARISON ".center(100, "="))
        self._print("=" * 100)

        compact = self.cg_solver.problem

        # Check if model is already built
        if compact.Model is None:
            self._print("⚠️  Compact model not built. Building now...")
            compact.buildModel()

        self._print(f"Compact model statistics:")
        self._print(f"  Variables:   {compact.Model.NumVars}")
        self._print(f"  Constraints: {compact.Model.NumConstrs}")
        self._print(f"  Nonzeros:    {compact.Model.NumNZs}")

        # Set solving parameters
        compact.Model.Params.OutputFlag = 1
        compact.Model.Params.TimeLimit = 3600  # 1 hour time limit
        compact.Model.Params.MIPGap = 0.0001  # 0.01% gap
        compact.Model.Params.Threads = -1  # Use all available threads

        self._print("\nStarting compact model optimization...")
        self._print("This may take a while for large instances...\n")
        ...
        try:
            ...
            if compact.Model.status == gu.GRB.OPTIMAL:
                results['obj_val'] = compact.Model.objVal
                results['obj_bound'] = compact.Model.objBound
                results['gap'] = 0.0
                results['nodes_explored'] = compact.Model.NodeCount

                self._print("\n✅ COMPACT MODEL SOLVED TO OPTIMALITY")
                self._print(f"   Objective:     {results['obj_val']:.6f}")
                self._print(f"   Solve time:    {solve_time:.2f}s")
                self._print(f"   Nodes:         {results['nodes_explored']}")

            elif compact.Model.status == gu.GRB.TIME_LIMIT:
                if compact.Model.SolCount > 0:
                    results['obj_val'] = compact.Model.objVal
                    results['obj_bound'] = compact.Model.objBound
                    results['gap'] = compact.Model.MIPGap
                    results['nodes_explored'] = compact.Model.NodeCount

                    self._print("\n⏱️  COMPACT MODEL HIT TIME LIMIT")
                    self._print(f"   Best solution: {results['obj_val']:.6f}")
                    self._print(f"   Best bound:    {results['obj_bound']:.6f}")
                    self._print(f"   Gap:           {results['gap']:.4%}")
                    self._print(f"   Solve time:    {solve_time:.2f}s")
                    self._print(f"   Nodes:         {results['nodes_explored']}")
                else:
                    self._print("\n⚠️  COMPACT MODEL: Time limit, no feasible solution found")
                    results['obj_val'] = float('inf')

            elif compact.Model.status == gu.GRB.INFEASIBLE:
                self._print("\n❌ COMPACT MODEL IS INFEASIBLE")
                results['obj_val'] = float('inf')

            else:
                self._print(f"\n⚠️  COMPACT MODEL: Unexpected status {compact.Model.status}")
                if compact.Model.SolCount > 0:
                    results['obj_val'] = compact.Model.objVal
                    results['obj_bound'] = compact.Model.objBound
                    results['gap'] = compact.Model.MIPGap
    ...
    def compare_with_compact_model(self, compact_results):
        """
        Compare B&P results with compact model results.

        Args:
            compact_results: Results dict from solve_compact_model_for_comparison()
        """
        self._print_always("\n" + "=" * 100)
        self._print_always(" B&P vs. COMPACT MODEL COMPARISON ".center(100, "="))
        self._print_always("=" * 100 + "\n")

        # Prepare B&P results
        bnp_obj = self.incumbent if self.incumbent < float('inf') else None
        bnp_lb = self.best_lp_bound if self.best_lp_bound < float('inf') else None
        bnp_time = self.stats['total_time']
        bnp_nodes = self.stats['nodes_explored']

        # Prepare Compact results
        compact_obj = compact_results.get('obj_val')
        compact_lb = compact_results.get('obj_bound')
        compact_time = compact_results.get('solve_time', 0)
        compact_nodes = compact_results.get('nodes_explored', 0)

        # Print comparison table
        self._print_always("┌─────────────────────────────┬──────────────────┬──────────────────┐")
        self._print_always("│ Metric                      │ Branch-and-Price │ Compact Model    │")
        self._print_always("├─────────────────────────────┼──────────────────┼──────────────────┤")

        # Objective Value
        bnp_obj_str = f"{bnp_obj:.6f}" if bnp_obj else "None"
        compact_obj_str = f"{compact_obj:.6f}" if compact_obj and compact_obj < float('inf') else "None"
        self._print_always(f"│ Objective Value (UB)        │ {bnp_obj_str:>16} │ {compact_obj_str:>16} │")

        # Lower Bound
        bnp_lb_str = f"{bnp_lb:.6f}" if bnp_lb else "None"
        compact_lb_str = f"{compact_lb:.6f}" if compact_lb and compact_lb > -float('inf') else "None"
        self._print_always(f"│ Lower Bound                 │ {bnp_lb_str:>16} │ {compact_lb_str:>16} │")

        # Gap
        bnp_gap_str = f"{self.gap:.4%}" if self.gap < float('inf') else "∞"
        compact_gap = compact_results.get('gap', float('inf'))
        compact_gap_str = f"{compact_gap:.4%}" if compact_gap < float('inf') else "∞"
        self._print_always(f"│ Gap                         │ {bnp_gap_str:>16} │ {compact_gap_str:>16} │")

        # Solve Time
        self._print_always(f"│ Solve Time (s)              │ {bnp_time:>16.2f} │ {compact_time:>16.2f} │")

        # Speedup
        if compact_time > 0 and bnp_time > 0:
            speedup = compact_time / bnp_time
            speedup_str = f"{speedup:.2f}x"
        else:
            speedup_str = "N/A"
        self._print_always(f"│ Speedup (Compact/B&P)       │ {speedup_str:>16} │                  │")

        # Nodes Explored
        self._print_always(
            f"│ Nodes Explored              │ {bnp_nodes:>16} │ {compact_nodes if compact_nodes else 'N/A':>16} │")

        self._print_always("└─────────────────────────────┴──────────────────┴──────────────────┘")

        # Optimality Check
        self._print_always("\n" + "─" * 100)
        self._print_always(" OPTIMALITY CHECK ".center(100, "─"))
        self._print_always("─" * 100 + "\n")

        if bnp_obj and compact_obj and compact_obj < float('inf'):
            diff = abs(bnp_obj - compact_obj)

            if diff < 1e-4:
                self._print_always("✅ OBJECTIVES MATCH - B&P found optimal solution!")
                self._print_always(f"   Difference: {diff:.8f} (within tolerance)")
            elif diff < 0.01:
                self._print_always("⚠️  OBJECTIVES CLOSE - Small difference detected")
                self._print_always(f"   B&P:     {bnp_obj:.6f}")
                self._print_always(f"   Compact: {compact_obj:.6f}")
                self._print_always(f"   Diff:    {diff:.6f} ({diff / compact_obj:.4%})")
            else:
                self._print_always("❌ OBJECTIVES DIFFER - Investigation needed!")
                self._print_always(f"   B&P:     {bnp_obj:.6f}")
                self._print_always(f"   Compact: {compact_obj:.6f}")
                self._print_always(f"   Diff:    {diff:.6f} ({diff / compact_obj:.4%})")

                if bnp_obj < compact_obj:
                    self._print_always("\n   ⚠️  B&P found BETTER solution than compact model!")
                    self._print_always("       This should not happen - check compact model formulation.")
                else:
                    self._print_always("\n   ⚠️  Compact model found BETTER solution!")
                    self._print_always("       B&P may not have found optimal - check convergence.")
        else:
            self._print_always("⚠️  Cannot compare - one or both solves did not finish successfully")
